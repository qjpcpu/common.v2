...
func NewComplexSelectWithHints(initialIndex int, items, hints []string) *ComplexSelect {
	list := NewComplexSelect(initialIndex, items)
	for i := range items {
...
func (sl *ComplexSelect) repaint(offset int) {
...
func (slist *ComplexSelect) createUI() {
	ls := termui.NewList()
...
func (sl *ComplexSelect) doSearch() {
	sl.search.SearchResultsIndices = []int{}
	sl.search.SelectedResultIndex = 0
	for i, c := range sl.items {
		if _, idx := py.FuzzyContain(strings.ToLower(c), strings.ToLower(sl.search.QueryStr)); idx >= 0 {
...
func (sl *ComplexSelect) appendQuery(qs string) {
	sl.search.QueryStr += qs
	sl.doSearch()
}

func (sl *ComplexSelect) reset() {
	sl.mode = ModeNorm
	sl.resetSearch()
	sl.uilist.BorderLabel = sl.title
}

func (slist *ComplexSelect) handleKeyboardEvents() {
...
func (sl *ComplexSelect) closeList() {
	sl.termWriter.Stop()
}

func (slist *ComplexSelect) writeTermLoop() {
...
func (sl *ComplexSelect) resetSearch() {
...
func (sl *ComplexSelect) highlight(raw string, hint string, background bool) string {
	so := sl.search
	...
	for loop := true; loop; loop = false {
		// search mode
		if matched, start := py.FuzzyContain(raw1, qs); sl.InSearchMode() && start != -1 {
			if start < 0 {
				if background {
...
func (slist *ComplexSelect) formatCommands() []string {
